...
import { Domain, EmailAccount } from '@/types/hosting';
...
const HostingDetail = () => {
  ...
  const { getHostingById, deleteHosting } = useHostings();
  const hosting = getHostingById(id!);
  ...
  const handleDelete = async () => {
    if (window.confirm('Вы уверены, что хотите удалить этот хостинг?')) {
      const success = deleteHosting(hosting!.id);
      if (success) {
        toast({
              ...
              <TabsContent value="domains" className="space-y-6">
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      {hosting.servers.reduce((acc, server) => acc + server.domains.length, 0)} доменов
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="grid gap-4">
                      ...
                      {hosting.servers.reduce((acc, server) => acc + server.domains.length, 0) === 0 && (
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      {hosting.servers.reduce((acc, server) => acc + server.emailAccounts.length, 0)} почтовых ящиков
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="grid gap-4">
                      ...
                      {hosting.servers.reduce((acc, server) => acc + server.emailAccounts.length, 0) === 0 && (
                        <div className="text-center py-8 text-muted-foreground">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4 text-center">
                  <div className="p-3 bg-muted/30 rounded-lg">
                    <div className="text-2xl font-bold">{hosting.servers.length}</div>
                  ...
                  <div className="p-3 bg-muted/30 rounded-lg">
                    <div className="text-2xl font-bold text-green-600">
                      {hosting.servers.filter(s => s.status === 'online').length}
                    </div>
